...
export let checkChatScroll = function () {
  ...
  // 스크롤이 최하단 근처 일때만 고정
  if (Math.abs(chatArea.scrollHeight - eh) <= 30) {
...
let clickChatRoom = function () {
  $(document).on("click", ".chat-room-item, .chat-all-user", function () {
    ...
    if ($(this).attr("id") == "chat-all-user") {
      ...
      id = $(this).children(".room-name").text();
    }

    if (id == "chatLogAll") {
      ChatData.showChattings(id);
    } else {
      if (ChatData.isEmpty(Common.getCookie("chatLog-" + id))) {
        // 로그가 비어있다면
        ChatData.getChattings(id, "bottom");
      } else {
        ChatData.showChattings("chatLog-" + id);
      }
    }
  });
};
...
export let setChattingRooms = function () {
