  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      document.querySelectorAll('.page-btn').forEach(function (el) {
        console.log(el);
        if (el.innerText == num) {
          //만약 페이지 넘버와 현재 버튼의 text가 같다면 해당 버튼 선택효과 적용
          console.log(el.className);
          el.className = 'selected-btn page-btn';
        }
      });
    },
...
let init = function () {
  ...
  document.getElementById('next-btn').addEventListener('click', function () {
    ...
    if (document.querySelectorAll('.disable-btn').length > 0) {
      // 만약 비활성화 된 버튼이 있다면
      //가장 첫번째 비활성화된 버튼의 앞요소 클릭처리
      document.querySelectorAll('.disable-btn')[0].previousSibling.click();
    } else {
      // 비활성화 된 버튼이 없다면 가장 마지막 버튼 요소 클릭 처리
      btns[btns.length - 1].click();
    }
  });
};
